...
def _plot_cbf_agent_mean_min(agent_dfs: dict[int, pd.DataFrame],
                             out_path: str,
                             episode_name: str,
                             mode: str = "mean") -> None:
    """Plot agent-axis aggregate CBF barrier values for one episode.
    ...
    Args:
        ...
        mode: "mean" or "min".
              "mean" plots average CBF value over agents at each step.
              "min" plots minimum CBF value over agents at each step.
    """
    ...
    fig.suptitle(f"{episode_name} — {mode_name} CBF values", fontsize=11)
    ...
    for row_idx, (col, ylabel, title) in enumerate(CBF_ROWS):
        ...
        for agent_idx, df in sorted(agent_dfs.items()):
            if col not in df.columns:
                continue

            # Basic length check. If logs are inconsistent, truncate to common length.
            vals = df[col].to_numpy(dtype=float)
            vals_per_agent.append(vals)
        ...
        steps_common = steps[:min_len]
        value_mat = np.stack([v[:min_len] for v in vals_per_agent], axis=0)

        if mode == "mean":
            agg_vals = np.mean(value_mat, axis=0)
        else:
            agg_vals = np.min(value_mat, axis=0)
...
def plot_episode(ep_dir: str, out_dir: str,
    ...
    selected_agent_dfs = {
        idx: df
        for idx, df in agent_dfs.items()
        if agents is None or idx in agents
    }

    mean_out_path = os.path.join(plot_dir, "cbf_agent_axis_mean.png")
    min_out_path = os.path.join(plot_dir, "cbf_agent_axis_min.png")

    _plot_cbf_agent_mean_min(
        selected_agent_dfs,
        mean_out_path,
        episode_name,
        mode="mean",
