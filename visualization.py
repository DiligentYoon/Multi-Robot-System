...
def draw_frame(ax_gt, ax_belief, viz_data: dict,
    ...
    if viz_data.get("targets_prob_heat") is not None:
        ...
        heat_norm_ma = np.ma.masked_invalid(heat_norm)

        # 0~1 값을 무지개 색으로 직선 매핑
        im_bf = ax_belief.imshow(
            heat_norm_ma,
            cmap="turbo",        # 무지개 계열. 더 클래식하게는 "rainbow" 도 가능
            vmin=0.0,
            vmax=1.0,
            alpha=0.8,
            origin="upper",
    ...
    # --- Assigned A* Destinations ---
    assigned_dests = viz_data.get("assigned_dests")
    if assigned_dests is not None and len(assigned_dests) > 0:
        for i in range(len(assigned_dests)):
            dest_rc = assigned_dests[i]
            # dest_rc is (row, col), convert to world then to image coords
            dest_world_x, dest_world_y = maps.grid_to_world(dest_rc[1], dest_rc[0])
            dest_img_x, dest_img_y = world_to_img(dest_world_x, dest_world_y)
            color = AGENT_COLORS[i % len(AGENT_COLORS)]
            for ax in (ax_gt, ax_belief):
